"""
Dashboard de Refacciones TEC - Streamlit App
...
st.set_page_config(
    page_title="Dashboard Refacciones TEC",
    page_icon="🔧",
    layout="wide",
)
...
total_costo     = filt["Costo"].sum()
total_ordenes   = filt["Order"].nunique()
total_equipos   = filt["Equipment"].nunique()
costo_promedio  = filt["Costo"].mean()
...
with col_table:
    st.markdown("#### Detalle")
    display_df = critical[["Refaccion", "Costo_Total_fmt", "Frecuencia", "% del Total"]].copy()
    display_df.columns = ["Refacción", "Costo Total", "Frecuencia", "% Total"]
    st.dataframe(display_df, hide_index=True, use_container_width=True, height=460)
...
with col_tipo:
    ...
    fig_pie = px.pie(
        by_tipo, values="Costo", names="TIPO",
...
fig_scatter = px.scatter(
    scatter_df.head(100),
    x="Frecuencia", y="Costo_Promedio",
    size="Costo_Total",
    color="Criticidad",
    color_continuous_scale="RdYlGn_r",
    hover_name="Refaccion",
...
st.markdown("---")
st.caption("Dashboard desarrollado con Streamlit + Plotly · Datos: info_TEC_limpio.xlsx")